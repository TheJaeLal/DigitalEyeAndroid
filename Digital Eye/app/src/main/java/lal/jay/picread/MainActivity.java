...
public class MainActivity extends AppCompatActivity {
    ...
    private byte[] decodedResponseImage;

    private static final int numDigitsInCaptionLength = 4;
    //Buttons..
...
//    private Button captureButton;
    ...
    private static String defaultImageQuality = "50";

    private static String defaultIPAddress = "35.200.202.208";
//    private static String defaultIPAddress = "192.168.43.147";

    private static int ObjectDetectionPortNo = 5000;
    private static int ImageCaptioningPortNo = 5001;
...
//        captureButton = findViewById(R.id.btn_capture);
        personButton = findViewById(R.id.btn_person);
        sceneButton = findViewById(R.id.btn_scene);
...
//            captureButton.setLayoutParams(layoutParams);
            ...
            sceneButton.setVisibility(View.INVISIBLE);
...
//        String response = uploadImage(bitmap2String(bitmap));
        ...
        sendForDetection(bitmap2String(bitmap,getImageQuality()));
...
//        }
    ...
    public void sendForCaptioning(final String image)
    {
        progressDialog.setMessage("Describing Image...");

        //Resolve server URL
        String serverUrl = getServerUrl("CAPTIONING");
...
//                Log.d("Network", "Response :"+response.length());
                ...
                Log.d("CAPTION:",response);
                String caption = response.substring(9,response.length()-7);
                Glide.with(MainActivity.this)
                        .load(decodedResponseImage)
                        .into(imageView);
...
//                uploaded = true;
    ...
    public void sendForDetection(final String image)
    {
        ...
        //Resolve server URL
        String serverUrl = getServerUrl("DETECTION");
...
//                Log.d("Network", "Response :"+response.length());
                ...
                decodedResponseImage = Base64.decode(response, Base64.DEFAULT);

                sendForCaptioning(image);

//
//
////            String captionLengthStr = response.substring(0, Math.min(response.length(), numDigitsInCaptionLength));
////
////            int captionLengthInt = Integer.parseInt(captionLengthStr);
////
////            Log.d("CaptionLength:", String.valueOf(captionLengthInt));
////
////            int imageStartIndex = numDigitsInCaptionLength + captionLengthInt;
////
////            String caption = response.substring(numDigitsInCaptionLength,captionLengthInt+4);
////
////            Log.d("Caption:",caption);
////
////            Log.d("ImageStart",response.substring(imageStartIndex,imageStartIndex+5));
////
////            String encodedResponseImage = response.substring(imageStartIndex,response.length());
////
////            byte[] decodedResponse = Base64.decode(encodedResponseImage, Base64.DEFAULT);
//
//
//                Glide.with(MainActivity.this)
...
//        String serverIPAddress = sharedPref.getString("pref_server_ip",defaultIPAddress);
        ...
        String serverIPAddress = defaultIPAddress;

        int portNo;

        if(requestType.toUpperCase().equals("DETECTION"))
        {
            Log.d("Request","OBJECT_DETECTION_REQUEST");
            portNo = ObjectDetectionPortNo;
        }
        else{
            Log.d("Request","IMAGE_CAPTIONING_REQUEST");
            portNo = ImageCaptioningPortNo;
        }
...
//        String value = sharedPref.getString("pref_image_quality",defaultImageQuality);
        ...
        String value = defaultImageQuality;
...
//        captureButton.setVisibility(View.VISIBLE);
    ...
    public void identifyPerson(View view) {

        //TODO: Potential Error Handling if the App/Package is not installed..
        Intent i = getPackageManager().getLaunchIntentForPackage("com.google.android.cameraview.demo");
        startActivity(i);
    }
}
